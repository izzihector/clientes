...
class AccountMove(models.Model):
    ...
    def action_post(self):
        ...
        if self.usar_anticipo==True:
            ...
            for det in self.payment_ids:
                ...
                self.concilio_saldo_pendiente(idv_move,det,cont,acum)
    ...
    def button_draft(self):
        ...
        #raise UserError(_('Mi Bebe:%s')%self.type)
        if self.type!="entry":
            #raise UserError(_('Mi Bebe:%s')%self.payment_id.anticipo)
            if self.payment_id.anticipo==True:
                cursor_payment = self.env['account.payment'].search([('id','=',self.payment_id.id)])
                #raise UserError(_('Mi Bebe:%s')%self.payment_id.id)
                for det_payment in cursor_payment:
                    id_mov_anti=det_payment.anticipo_move_id.id
                    #raise UserError(_('Mi Bebe:%s')%det_payment.anticipo_move_id.amount_total)
                ...
                for det_anticipo in cursor_anticipo:
                    self.env['account.move'].browse(det_anticipo.id).write({
                                    'state':"cancel",
                                    })



    def concilio_saldo_pendiente(self,id_move_conci,id_payment,cont2,acum_anti):
        ...
        #if cont2==3:
            #raise UserError(_('total anticipo = %s')%factura_monto)
    ...
    #bebe = fields.Char(string='darrell')
