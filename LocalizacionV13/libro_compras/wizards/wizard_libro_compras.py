...
class libro_compras(models.TransientModel):
    ...
    def generate_xls_report(self):
        ...
        ws1.write_merge(row, row, 2, 4,  str(self.company_id.partner_id.doc_type) +'-'+ str(self.company_id.partner_id.vat), sub_header_content_style)
        row+=1
        ws1.write_merge(row, row, 0, 1, "Direccion Fiscal:", sub_header_style)
        ws1.write_merge(row, row, 2, 5, str(self.company_id.street), sub_header_content_style)
        row +=1
        ...
        row += 1
        ws1.write(row,col+0,"#",sub_header_style)
        ws1.col(col+0).width = int((len('Desde :')+1)*256)
        ws1.write(row,col+1,"Fecha Documento",sub_header_style)
        ws1.col(col+1).width = int((len('Fecha Documento')+3)*256)
        ws1.write(row,col+2,"RIF",sub_header_style)
        ws1.col(col+2).width = int((len('J-456987531'))*256)
        ws1.write(row,col+3,"Nombre Razon Social",sub_header_style)
        ws1.col(col+3).width = int(len('Nombre Razon Social')*256)
        ws1.write(row,col+4,"Tipo de Persona",sub_header_style)
        ws1.col(col+4).width = int(len('Tipo de Persona')*256)
        ws1.write(row,col+5,"Nro. Factura / Entrega",sub_header_style)
        ws1.col(col+5).width = int(len('Nro. Factura / Entrega')*256)
        ws1.write(row,col+6,"Nro de control",sub_header_style)
        ws1.col(col+6).width = int(len('Nro de control')*256)
        ws1.write(row,col+7,"Nro de nota de debito",sub_header_style)
        ws1.col(col+7).width = int(len('Nro de nota de debito')*256)
        ws1.write(row,col+8,"Nro de nota de credito",sub_header_style)
        ws1.col(col+8).width = int(len('Nro de nota de credito')*256)
        ws1.write(row,col+9,"Nro de factura afectada",sub_header_style)
        ws1.col(col+9).width = int(len('Nro de factura afectada')*256)
        ws1.write(row,col+10,"Tipo de transacc.",sub_header_style)
        ws1.col(col+10).width = int(len('Tipo de transacc.')*256)
        ws1.write(row,col+11,"Nro Planilla de Importaciones",sub_header_style)
        ws1.col(col+11).width = int(len('Nro Planilla de Importaciones')*256)
        ws1.write(row,col+12,"Nro Expediente Importaciones",sub_header_style)
        ws1.col(col+12).width = int(len('Nro Expediente Importaciones')*256)
        ws1.write(row,col+13,"Fecha de importaciones",sub_header_style)
        ws1.col(col+13).width = int(len('Fecha de importaciones')*256)
        ws1.write(row,col+14,"Total compras con IVA",sub_header_style)
        ws1.col(col+14).width = int(len('Total compras con IVA')*256)
        ws1.write(row,col+15,"Exentas",sub_header_style)
        ws1.write(row,col+16,"Exoneradas",sub_header_style)
        ws1.write(row,col+17,"Base Imponible",sub_header_style)
        ws1.col(col+17).width = int(len('Base Imponible')*256)
        ws1.write(row,col+18,"'%'Alic",sub_header_style)
        ws1.col(col+18).width = int(len('IVA (00.0%) compras')*256)
        ws1.write(row,col+19,"Impuesto IVA",sub_header_style)
        ws1.col(col+19).width = int((len('Impuesto IVA')+2)*256)
        ws1.write(row,col+20,"IVA Retenido (Vendedor)",sub_header_style)
        ws1.col(col+20).width = int(len('IVA Retenido (Vendedor)')*256)
        ws1.write(row,col+21,"Nro. Comprobante de retencion",sub_header_style)
        ws1.col(col+21).width = int(len('Nro. Comprobante de retencion')*256)
        ws1.write(row,col+22,"Fecha comp.",sub_header_style)
        ...
        numero = 1
        lines= self.line.sorted(key=lambda x: (x.name,x.invoice_number ),reverse=False)
        for invoice in lines:
            ...
            ws1.write(row,col+2,str(invoice.document),center)
            ...
            elif invoice.tipo_doc == 'nc' or invoice.tipo_doc == 'nb':
        ...
        ws1.write(row,col+14,invoice.float_format(total_con_IVA),sub_header_style)
        ws1.write(row,col+15,invoice.float_format(tot_exentas),sub_header_style)
        ws1.write(row,col+16,invoice.float_format(tot_exoneradas),sub_header_style)
        ws1.write(row,col+17,invoice.float_format(total_imponible),sub_header_style)
        #ws1.write(row,col+19,'0',sub_header_style)
        ws1.write(row,col+19,invoice.float_format(total_iva),sub_header_style)
        ws1.write(row,col+20,invoice.float_format(reten_iva_total),sub_header_style)
        ...
        for l in self.line:
            if l.alicuota_type == 'exempt':
                if l.tipo_doc == 'nc':
                    ...
                    if l.state_retantion == 'posted':
                        tot_reten_exent -= float(l.iva_retenido)
                else:
                    tot_base_exentas += float(l.base_imponible)
                    tot_iva_exentas += float(l.iva)
                    if l.state_retantion == 'posted':
        ...
        for l in self.line:
            if l.alicuota_type == 'no_tax_credit':
                if l.tipo_doc == 'nc':
                    ...
                    if l.state_retantion == 'posted':
                        tot_reten_no_gra -= float(l.iva_retenido)
                else:
                    tot_base_no_gra += float(l.base_imponible)
                    tot_iva_no_gra += float(l.iva)
                    if l.state_retantion == 'posted':
        ...
        row +=2
        ws1.write_merge(row, row, 12, 16,"Resumen de Compras",sub_header_style)
        ws1.write(row,col+17,"Base Imponible",sub_header_style)
        ...
        row+=1
        ws1.write_merge(row, row, 12, 16,"Compras internas Exoneradas",sub_header_style)
        ...
        ws1.write(row, col + 17,invoice.float_format((base_general+base_genel_mas_adicional+base_reducido+tot_base_exentas+tot_base_no_gra)), result_right)
        ws1.write(row, col + 18,invoice.float_format((tax_general+tax_genel_mas_adicional+tax_reducido+tot_iva_exentas+tot_iva_no_gra)), result_right)
        ws1.write(row, col + 19,invoice.float_format((ret_general+ret_genel_mas_adicional+ret_reducido+tot_reten_exent+tot_reten_no_gra)), result_right)

        wb1.save(fp)
        out = base64.encodestring(fp.getvalue())
        self.write({'state': 'get', 'report': out, 'name':'Resumen_de_Compras.xls'})
        return {
