...
class sale_day_book_wizard(models.TransientModel):
    ...
    warehouse = fields.Many2many('stock.warehouse', 'wh_wiz_rel_inv_val', 'wh', 'wiz', string='Almacen')
    ...
    filter_by = fields.Selection([('product','Product'),('categ','Category')],string="Filtro Por",default = "product")
    product_ids = fields.Many2many('product.product', 'rel_product_val_wizard', string="Producto")
    ...
    def print_exl_report(self):
        if xlwt:
            ...
            if self.display_sum:
                worksheet.write_merge(0, 1, 1, 6, "Informe de resumen de valoración de inventario", style=style_title)
                worksheet.write(8, 0, 'Categoria', style_table_header)
                worksheet.write(8, 1, 'Inicial', style_table_header)
                ...
                worksheet.write_merge(0, 1, 2, 9, "Informe de valoración de inventario", style=style_title)
                worksheet.write(8, 0, 'Codigo', style_table_header)
                worksheet.write(8, 1, 'Nombre', style_table_header)
                worksheet.write(8, 2, 'Categoria', style_table_header)
                worksheet.write(8, 3, 'Metodo de Costeo', style_table_header)
