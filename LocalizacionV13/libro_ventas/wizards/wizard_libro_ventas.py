...
class libro_ventas(models.TransientModel):
    ...
    def generate_xls_report(self):
        ...
        ws1.write(row,col+8,"Nro de nota de debito",sub_header_style)
        ws1.col(col+8).width = int(len('Nro de nota de debito')*256)
        ws1.write(row,col+9,"Numero de nota de credito ",sub_header_style)
        ...
        ws1.write(row,col+11,"Tipo de transacc.",sub_header_style)
        ws1.col(col+11).width = int(len('Tipo de transacc.')*256)
        ws1.write(row,col+12,"Ventas Incluyendo el IVA",sub_header_style)
        ws1.col(col+12).width = int(len('Ventas Incluyendo el IVA')*256)
        ws1.write(row,col+13,"Ventas internas o exoneraciones no gravadas",sub_header_style)
        ws1.col(col+13).width = int(len('Ventas internas o exoneraciones no gravadas')*256)
        ws1.write(row,col+14,"Ventas internas o exportaciones exoneradas",sub_header_style)
        ws1.col(col+14).width = int(len('Ventas internas o exportaciones exoneradas')*256)
        ws1.write(row,col+15,"Base Imponible",sub_header_style)
        ws1.col(col+15).width = int(len('Base Imponible')*256)
        ws1.write(row,col+16,"'%'Alicuota",sub_header_style)
        ws1.col(col+16).width = int((len('IVA (00.0%) ventas')+1)*256)
        ws1.write(row,col+17,"Impuesto IVA",sub_header_style)
        ws1.col(col+17).width = int((len('Impuesto IVA')+2)*256)
        ws1.write(row,col+18,"IVA Retenido Comprador",sub_header_style)
        ws1.col(col+18).width = int(len('IVA Retenido Comprador')*256)
        ws1.write(row,col+19,"Nro. Comprobante de Retencion",sub_header_style)
        ws1.col(col+19).width = int(len('Nro. Comprobante de retencion')*256)
        ws1.write(row,col+20,"Fecha comp",sub_header_style)
        ...
        ws1.write(row,col+12,total_con_iva,sub_header_style) #Total con IVA
        ws1.write(row,col+13,tot_no_gra,sub_header_style) # EXONERADAS NO AGRAVADAS
        ws1.write(row,col+14,tot_exentas,sub_header_style) # exoneradas
        ws1.write(row,col+15,total_bi,sub_header_style)# base imponible
        ws1.write(row,col+17,total_iva,sub_header_style) #iva
        ws1.write(row,col+18,total_iva_reten,sub_header_style) #retencion de iva
        ...
        row +=2
        ws1.write_merge(row, row, 11, 15,"Resumen de Ventas",sub_header_style)
        ...
        self.write({'state': 'get', 'report': out, 'name':'Resumen_de_Ventas.xls'})
        return {
